...
import { getAllUser } from '../../utils/fetchFromAPI';
...
const Footer = () => {
    ...
    useEffect(() => {
        ...
        socket.on('sv-send-mess',({user_id,content})=>{
            ...
            setDataChat(newData);
        })// nhớ sửa lại cái gia giá trị nhận vô với useEffect, nói chung thì nhớ sửa lại, với có thể có mã hóa
        ...
        <div className="chat-popup" id="formChat">
            ...
            <ol className="discussion" id="chat-noiDung">
                {dataChat?.map((item,index)=>{
                    ...
                    // WELL, THỰC RA CÁI NÀY KHÁ NHẢM VI VÌ LƯU USER_ID RỒI
...
{/*  */}
            ...
            <div className="chatBottom">
                ...
                <button id="btn-send" onClick={() => {
                        // lấy userId từ trong localstorage;
                        // let userlogin = localStorage.getItem('LOGIN_USER');
                        let user_id = localStorage.getItem('user_id');
                        // // decode thử xem thằng loginuser trả về cái khỉ gì
                ...
                }} type="button" className="sendbtn" aria-label="Close"><span aria-hidden="true"> Send <i className="fa-regular fa-paper-plane"></i></span></button>
            </div>
